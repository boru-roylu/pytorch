...
void RNNCellImplBase<Derived>::pretty_print(std::ostream& stream) const {
  std::string s = "{input_size}, {hidden_size}";
  if (!options_base.bias()) {
    s += ", bias={bias}";
  }
  ...
  s = std::regex_replace(s, std::regex("\\{input_size\\}"), c10::str(options_base.input_size()));
  s = std::regex_replace(s, std::regex("\\{hidden_size\\}"), c10::str(options_base.hidden_size()));
  s = std::regex_replace(s, std::regex("\\{bias\\}"), c10::str(options_base.bias()));
  s = std::regex_replace(s, std::regex("\\{nonlinearity\\}"), c10::str(nonlinearity_str));

  const std::string name = this->name();
  const std::string name_without_impl = name.substr(0, name.size() - 4);
  stream << name_without_impl << "(" << s << ")";
}
...
template <typename Derived>
std::string RNNCellImplBase<Derived>::get_nonlinearity_str() const { // yf225 TODO: overwrite this in RNNCell case
  return "";
}
